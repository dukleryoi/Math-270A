...
int main()
{
	...
	double dt = 4 * h*h / (alphaX*pi1*pi1);
	...
	GridFun2D uk_ref(xPanel, xMin, xMax, yPanel, yMin, yMax);
	...
	double uNorm;
	double dt_ref = dt;
	int smax = int(log2(maxPanel)) + 1;
	vector<RelaxOp2D> relaxOps(smax);
	for (int i = 0; i < smax; i++)
	{
		(relaxOps[i]).initialize(dt, alphaX, alphaY, f);
		dt *= 4;
	}
	...
	while ((diffNorm > tol) && (iter < iterMax))
	{
		...
		{
			(relaxOps[i]).apply(uk, ukp1);
			uk = ukp1;
		}

		for (long i = smax - 1; i > -1; i--)
		{
			(relaxOps[i]).apply(uk, ukp1);
			uk = ukp1;
		}
